...
public class Board {
        ...
        for (int r = 0; r < grid.getRows(); r++) {
            for (int c = 0; c < grid.getCols(); c++) {
                if (isActive[r][c] && cells[r][c] != null && !cells[r][c].isBlue()) {
                    ...
                    // Дебаг для проблемной ячейки [2,0]
                    if (r == 2 && c == 0) {
                        System.out.println("Ячейка [2,0] имеет clue=" + clue + ", соседи проверены");
                    }
                    // Проверка на несоответствие
                    int actualNeighbors = countBlueNeighbors(r, c);
                    if (clue != actualNeighbors) {
                        System.err.println("Ошибка: ячейка [" + r + "," + c + "] имеет clue=" + clue + ", но синих соседей=" + actualNeighbors);
                    }
                }
            }
        }
    ...
    private int countBlueNeighbors(int r, int c) {
        int count = 0;
        int[] neighbors = grid.getNeighbors(r, c);
        // Дебаг для [2,0]
        if (r == 2 && c == 0) {
            System.out.print("Соседи ячейки [2,0]: ");
        ...
        for (int neighbor : neighbors) {
            if (neighbor != -1) {
                ...
                if (isActive[nr][nc] && cells[nr][nc] != null && cells[nr][nc].isBlue()) {
                    count++;
                    // Логируем только для [2,0]
                    if (r == 2 && c == 0) {
                        System.out.println("Синяя клетка найдена в соседе [" + nr + "," + nc + "] для [2,0]");
                    }
                }
            }
        }
